...
public class Warehouse {
    ...
    /**
     * TODO: Part 1d.
     * Get the list of locations of th8e given part.
    ...
    public PurchaseOrder createPurchaseOrder(ClientOrder clientOrder)
    {
        ...
        ItemInventory clientInventory = clientOrder.getItemInventory();
        for (Item item : clientInventory.getItems()){
            int partCode = item.getPartCode();
            int quantity = item.getQuantity();
            int partCount = getPartCount(partCode);
            if (partCount < quantity){
                int amountRequired = quantity - partCount;
                Item purchaseItem = new Item(partCode, amountRequired);
    ...
    public List<PickListItem> createAPickList(ClientOrder order)
    {
        ...
        // the order.
        ItemInventory clientInventory = order.getItemInventory();
        for (Item item : clientInventory.getItems()) {
            int partCode = item.getPartCode();
            ...
            for (Location location : locations) {
                ...
                int quantityToPick = Math.min(warehouseItem.getQuantity(), remainingQuantity);
                pickListItems.add(new PickListItem(location, new Item(partCode, quantityToPick)));
                warehouseItem.reduceQuantity(quantityToPick);
            ...
            if (locations.size() == 0){
                ...
                for (Location location : emptyLocations){
                    ...
                    if (quantity <= MAX_AMOUNT){
                        Item item = new Item(partCode, quantity);
                        addToWarehouse(location, item);
                ...
                for (Location location : locations){
                    ...
                    Item theItemInLocation = getItemAt(location);
                    if (theItemInLocation.getQuantity() < MAX_AMOUNT){
                        int amountToAdd = Math.min(MAX_AMOUNT - theItemInLocation.getQuantity(), quantity);
                        theItemInLocation.increaseQuantity(amountToAdd);
                        whereStored.add(location);
